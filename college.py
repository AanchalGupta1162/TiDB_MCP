...
log = logging.getLogger("mcp-tidb")
...
# MCP Server Setup
# -----------------------
mcp = FastMCP("tidb-mcp-server")

# --- Tool 1: Events in duration ---
@mcp.tool()
def get_events_in_duration(start_date: str, end_date: str) -> str:
    ...
    try:
        ...
        log.error(f"Error in get_events_in_duration: {e}")
        return f"Internal error: {e}"

# --- Tool 2: Events by type ---
@mcp.tool()
def get_events_by_type(event_type: str) -> str:
    """
    Retrieves all events of a specific type from the TiDB database.
    Args:
        event_type: The type of the event (e.g., 'exam', 'lecture').
    """
    ...
    try:
        ...
        log.error(f"Error in get_events_by_type: {e}")
        return f"Internal error: {e}"
...
if __name__ == "__main__":
    log.info("Starting MCP TiDB server...")
    # FastMCP does not accept host/port params in run(), only stdio
    mcp.run()